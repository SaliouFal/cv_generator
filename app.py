...
def build_placeholders(cv: dict) -> dict:
    ...
    ph: dict[str, str] = {
        ...
        "%%PHOTO_FILE%%"     : cv.get("photo", "example-image")
    }
    ...
    if skills:
        ...
        # grille 4×3 avec cercles blancs
    # ------------------------------------------------------------------
#  Grille 3 × 4 de compétences cerclées : %%SKILL_CIRCLES%%
    ...
    # on complète la dernière ligne pour avoir 4 colonnes pleines
    ...
    if not hobbies:                                    # rien ?  → valeur par défaut
        hobbies = ["Lectur",
                "Sports",
                "Musique", "Voyage"]
        cv["hobbies"] = hobbies

    # on formate en environnement itemize
    hobby_items = r"\begin{itemize}[leftmargin=*]" + "\n" + \
...
\colorbox{{maincolor}}{{%
  \begin{{minipage}}{{\linewidth}}
    \textbf{{{latex_escape(j.get('title',''))}}} \\ {latex_escape(j.get('company',''))} \\ {j.get('dates','') or ''}
    \begin{{itemize}}
      \item {bullet_items}
    \end{{itemize}}
  \end{{minipage}}}}"""
    ...
    for d in cv.get("degrees", []):
        ...
        # tableau à 2 colonnes (icône | texte extensible)
        deg_blocks.append(rf"""
    \begin{{tabularx}}{{\linewidth}}{{@{{}}c >{{\RaggedRight\arraybackslash}}X@{{}}}}
    \textcolor{{sidetext}}{{\faGraduationCap}} &
    \textbf{{{latex_escape(title)}}} \\
    & {latex_escape(school)} \\
    & \textit{{{latex_escape(dates)}}} \\
    \end{{tabularx}}
    {bullet_block}""")
    ...
    # ---- échappement final : **pas** pour les blocs LaTeX -------------------
    raw_tags = {
    "%%LANGUAGE_ITEMS%%",          # ← ajoute-les
    "%%SKILL_ITEMS%%",             # ← ici aussi
    "%%LANGUAGE_TABLE%%", "%%SKILL_ROWS%%",
    "%%EXPERIENCE_BLOCKS%%", "%%DEGREE_BLOCKS%%", "%%HOBBY_ITEMS%%","%%SKILL_CIRCLES%%"
}

    clean = {}
    for tag, val in ph.items():
        if tag in raw_tags:
            clean[tag] = val                       # déjà en LaTeX
        else:
            clean[tag] = latex_escape(val or "")
    return clean


def apply_placeholders(tex:str, ph:dict) -> str:
    for tag,val in ph.items(): tex = tex.replace(tag,val)
    print(tex)
...
def gpt_enrich(partial: dict) -> dict:
    ...
    # — Compétences -----------------------------------------------------------
    "• skills : [] La liste des compétences clés chaque compétences doit etre ecrit **un seul mot** chacun ; \n"
